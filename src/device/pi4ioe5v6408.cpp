...
esp_err_t Pi4ioe5v6408::Init(const I2cBus &bus, uint8_t dev_addr)
{
    if (m_handle != nullptr)
    {
...
esp_err_t Pi4ioe5v6408::SetDirection(uint32_t io_num, uint32_t direction)
{
    if (m_handle == nullptr)
    {
...
esp_err_t Pi4ioe5v6408::SetLevel(uint32_t io_num, uint32_t level)
{
    if (m_handle == nullptr)
    {
        m_logger.Error("IO expander not initialized");
...
esp_err_t Pi4ioe5v6408::GetLevel(uint32_t io_num, uint32_t *level)
{
    if (m_handle == nullptr)
    {
        m_logger.Error("IO expander not initialized");
...
esp_err_t Pi4ioe5v6408::SetPullupMode(uint32_t io_num, uint32_t pull_mode)
{
    if (m_handle == nullptr)
    {
        m_logger.Error("IO expander not initialized");
...
esp_err_t Pi4ioe5v6408::SetOutputMode(uint32_t io_num, esp_io_expander_output_mode_t mode)
{
    if (m_handle == nullptr)
    {
        m_logger.Error("IO expander not initialized");
...
esp_err_t Pi4ioe5v6408::PrintState()
{
    if (m_handle == nullptr)
    {
        m_logger.Error("IO expander not initialized");
    ...
    return esp_io_expander_print_state(m_handle);
}
